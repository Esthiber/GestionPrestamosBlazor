...
namespace GestionPrestamos.Services
{
    public class CuotasService(IDbContextFactory<Contexto> DbFactory)
    {
        ...
        public async Task<bool> AfectarCuotas(List<CobrosDetalle> cobrosDetalles)
        {
            using var context = await DbFactory.CreateDbContextAsync();

            foreach (var detalle in cobrosDetalles)
            {
                var PrestamoId = detalle.PrestamoId;
                var balance = detalle.ValorCobrado;
                if (PrestamoId <= 0 || balance < 0)
                {
                    throw new ArgumentException("PrestamoId debe ser mayor que 0 y balance no puede ser negativo.");
                }

                var detalleCuotas = await GetCuotasDetallesAsync(PrestamoId);
                double restante = balance;

                foreach (var cuota in detalleCuotas.Cuotas)
                {
